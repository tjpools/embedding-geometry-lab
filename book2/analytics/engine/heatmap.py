...
def framing_matrix(architecture: dict) -> Tuple[str, List[str], List[str], List[List[float]], List[List[str]]]:
    ...
    return "BOOK TWO ARCHITECTURE DENSITY", labels, columns, values, displays


def chapter_matrix(metrics: Sequence[dict]) -> Tuple[str, List[str], List[str], List[List[float]], List[List[str]]]:
    ...
    return "BOOK TWO CHAPTER ANALYTICS", labels, columns, values, displays
...
def write(path: Path, mode: CorpusMode, metrics: Sequence[dict], architecture: dict) -> None:
    title, labels, columns, values, displays = (
        framing_matrix(architecture) if mode == CorpusMode.FRAMING else chapter_matrix(metrics)
    )
    label_width = 300
    cell_width = 145
    row_height = 48
