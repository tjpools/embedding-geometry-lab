...
In retrospect, EasterDate was not an isolated project. It was an early rehearsal for the questions this book has been pursuing all along.

## 15.1 The Question: “What is the date of Easter?”

The question appears simple. It is the kind of question that invites a lookup table, a formula, or a quick search. But historically it was never merely a question of retrieval. It belonged to *computus*, the long tradition of determining Easter through lawful calendrical and astronomical structure. The moment the question is asked computationally, it changes shape.
...
And EasterDate was the first time that doorway opened.

## 15.2 Lookup Table or Algorithm

A lookup table gives answers.  
An algorithm gives structure.

Gauss’s Easter algorithm is not a list of dates. It is a compressed geometry of lunar cycle, solar calendar, and ecclesiastical rule. It does not store the answer in advance. It produces the answer by lawful transformation.

To implement such a procedure is to discover that the algorithm is not merely a recipe. It is a space, and the computation is a path through that space.

This is the moment when a program stops being a tool and becomes a world.

The distinction matters. A table preserves outcomes; an algorithm preserves relations. A table answers the question transactionally. An algorithm carries forward the structure of *computus* itself, rendering a historical problem into executable form. What survives in EasterDate is not merely the date of Easter for one year or another, but the patterned logic by which the date becomes determinable at all.

## 15.3 The Directory as World

The directory that held EasterDate — the `.asm` file, the build script, the notes, the compiled binary — became a kind of microcosm. It was not just storage. It was topology.

Inside that directory lived:
...
The directory was a world because it contained not just code, but the relations that made the code meaningful.

This was the first time a computational artifact appeared not merely as a file or program, but as a place. One could move through it, rebuild it, inspect it, and return to it. The directory became a container not only for implementation, but for intelligibility. It held source, mechanism, artifact, and explanation together in one bounded space.
